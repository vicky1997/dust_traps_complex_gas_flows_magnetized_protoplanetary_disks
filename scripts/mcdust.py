...
class Params :
    def __init__(self):
        ...
        self.ntime = None
        self.datadir = ''
    def read_params(self, path, hdf5=True,parfile='setup.par'):
        ...
        if (hdf5==True) :
            ...
            self.datadir = path
            nroutputs = len(glob.glob1(path,"*.out"))
...
class Simulation:
    def __init__(self):
        ...
        self.pars = []
        self.disk = []
    def read(self,path,hdf5=True, readatt=True):
        if(hdf5==True):
            self.pars = Params()
            ...
            sels.pars = Params()
            self.pars.read_params(path,hdf5=False)
            self.swarms = Swarm(self.pars)
            self.swarms.read_binary(self.pars)
...
class Plots:
    def rz_scatter(swarms, pars, time=-1, scatter=None, step=1, cmap = 'viridis', cbarlabel='') :
        ...
        if scatter.all() ==None :
            ...
            #ax.set_xscale('log')
        ax.set_xlim(pars.minr,pars.maxr)
