...
module hdf5output
  ...
  use parameters,  only: con2, matdens, vfrag, nbins, smallr, path, dumpfile
  use constants
  ...
  private
  public      :: hdf5_file_write, hdf5_file_t, hdf5_file_close, hdf5_file_read, hdf5_write_att, read_ncolls, write_ncolls, &
                                              write_dumpfile
  ...
  contains
  subroutine hdf5_file_write (this, swrm, time, mode, outnr, mswarm, Ntot, resdt)
    ...
    if (mode .eq. 'create') then
      call h5open_f(error)
      call hdf5_file_create(this, filepath, outnr , mswarm, Ntot, size(swrm), resdt)     ! creates file
  ...
  subroutine swarm_unit_conversion(swrm, tempswarm)
    ...
    do i=1,size(swrm)
      ...
      tempswarm(i)%velr = swrm(i)%velr
      tempswarm(i)%velz = swrm(i)%velz
    end do
...
! subroutine to add the swarm object as a compound datatype to the HDF5 file
  subroutine create_swarm_type(this,tempswrm)
    ...
    call h5tinsert_f(this%memtype, "id_number", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%idnr)), &
                                                                                  H5T_NATIVE_INTEGER, this%error)
    call h5tinsert_f(this%memtype, "mass_of_a_particle_[g]", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%mass)), &
                                                                                  H5T_NATIVE_DOUBLE, this%error)
    call h5tinsert_f(this%memtype, "cylindrical_radius_[AU]", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%rdis)), &
                                                                                  H5T_NATIVE_DOUBLE, this%error)
    call h5tinsert_f(this%memtype, "height_above_midplane_[AU]", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%zdis)), &
                                                                                  H5T_NATIVE_DOUBLE, this%error)
    call h5tinsert_f(this%memtype, "Stokes_number", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%stnr)), &
                                                                                  H5T_NATIVE_DOUBLE, this%error)
    call h5tinsert_f(this%memtype, "Radial_velocity_v_r_[cm/s]", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%velr)), &
                                                                                  H5T_NATIVE_DOUBLE, this%error)                                                                              
    call h5tinsert_f(this%memtype, "Vertical_velocity_v_z_[cm/s]", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%velz)), &
                                                                                  H5T_NATIVE_DOUBLE, this%error)
    call h5tinsert_f(this%memtype, "frag", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%frag)), &
                                                                                  H5T_NATIVE_INTEGER, this%error)
    call h5tinsert_f(this%memtype, "stick", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%stick)), &
                                                                                  H5T_NATIVE_INTEGER, this%error)
    call h5tinsert_f(this%memtype, "origin", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%origin)), &
                                                                                  H5T_NATIVE_INTEGER, this%error)
    return
  ...
  subroutine hdf5_file_create(this, filename, nout, mswarm, Ntot, nsize, resdt)
    implicit none
    type(hdf5_file_t), intent(out) :: this
    ...
    call hdf5_write_att(this%id, 'author(s)', 'Dr Joanna Drazkowska, Vignesh Vaikundaraman, Nerea Gurrutxaga')
    ...
    call hdf5_write_att(this%id, 'timestep[yr]', resdt)
    call hdf5_write_att(this%id, 'Nsize', nsize)
  ...
  subroutine hdf5_file_read(swrm, nout, mswarm, time, Ntot, resdt)
    implicit none
    ...
    integer :: error, i, nout, Nsize
    real, target :: mswarm, time, resdt
    integer, intent(inout) :: Ntot

    !call hdf5_read_att(nout,mswarm,Ntot)
    !write(*,*) nout, mswarm, Ntot
    ...
    call hdf5_read_att(nout,mswarm,Ntot, Nsize, resdt)
    write(*,*) nout, mswarm, Ntot, resdt/year, Nsize
    if (.not.allocated(swrm)) allocate( swrm(Nsize) )
    ...
    !call hdf5_read_att(nout,mswarm, Ntot)
    call h5close_f(error)
    write(*,*)'rmin: ', minval(swrm(:)%rdis)/AU, 'AU, rmax: ', maxval(swrm(:)%rdis)/AU, 'AU'
    return
  ...
  subroutine hdf5_read_att(nout, mswarm, Ntot, Nsize, resdt)
    implicit none
    type(hdf5_file_t) :: this
    type(C_PTR) :: f_ptr
    real, target :: mswarm, resdt
    integer, target :: Ntot, Nsize
    integer(HSIZE_T), dimension(1) :: dims
    integer(HID_T) :: att1, att2, att3, att4, att5
    integer, target :: nout
    ...
    CALL h5aopen_name_f(this%id, 'Nsize', att4, this%error)
    call check_error(this% error, 'error opening the attribute output_number')

    f_ptr = C_LOC(Nsize)
    CALL h5aread_f(att4, H5T_NATIVE_INTEGER, Nsize, dims, this%error)
    CALL h5aclose_f(att4, this%error)
    write(*,*) 'Nsize read'

    CALL h5aopen_name_f(this%id, 'timestep[yr]', att5, this%error)
    call check_error(this% error, 'error opening the attribute output_number')

    f_ptr = C_LOC(resdt)
    CALL h5aread_f(att5, H5T_NATIVE_DOUBLE, resdt, dims, this%error)
  ...
  subroutine write_ncolls(ncolls,ncolls_VB)
    implicit none
    integer, dimension(:,:), intent(in) :: ncolls, ncolls_VB
    integer, dimension(2) :: shape1, shape2
    integer :: i,j
 
    shape1 = shape(ncolls)
    shape2 = shape(ncolls_VB)
    open(11,file='ncolls.dat',status='REPLACE')
    do i =1,shape1(1)
      do j= 1, shape1(2)
        write(11,*) ncolls(i,j)
      enddo
    ...
    open(12,file='ncollsVB.dat',status='REPLACE')
    do i =1,shape2(1)
      do j= 1, shape2(2)
        write(11,*) ncolls_VB(i,j)
      enddo
    enddo
    close(12)

    end subroutine write_ncolls

  subroutine read_ncolls(ncolls,ncolls_VB)
    implicit none
    integer, dimension(:,:), intent(inout) :: ncolls,ncolls_VB
    integer, dimension(2) :: shape1, shape2
    integer :: i,j

    shape1 = shape(ncolls)
    shape2 = shape(ncolls_VB)
    open(1,file='ncolls.dat',action='read')
    do i =1,shape1(1)
      do j= 1, shape1(2)
        read(1,*) ncolls(i,j)
      enddo
    ...
    open(2,file='ncollsVB.dat',action='read')
    do i =1,shape2(1)
      do j= 1, shape2(2)
        read(2,*) ncolls_VB(i,j)
      enddo
    enddo
    close(2)
  end subroutine read_ncolls

  subroutine write_dumpfile(this, swrm, time, outnr,mswarm, Ntot, resdt)
    ...
    integer :: error
    !filename = 'swarms.h5'
    write(filename,'(a7)') 'dump.h5'
    !filepath = trim(path)//"/"//trim(filename)
    !write(*,*) filepath
    !call h5open_f(error)
    call h5open_f(error)
    call hdf5_file_create(this, filename, outnr , mswarm, Ntot, size(swrm), resdt)     ! creates file
    call hdf5_create_dataspace(this, swrm)                  ! creates dataspace and adds swarm  data
    timeout = time/year
    call hdf5_time_create(this, timeout)                    ! creates dataspace for time outputs
    call hdf5_file_close(this,this%swarms)
