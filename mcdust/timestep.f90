...
module timestep
   ...
   !use grid,       only: g
   use parameters, only: smallr, vertsett, rdrift, collswitch
   ...
   subroutine time_step(bin, ncolls, dtout, final_dtime, g)
      implicit none
      ...
      do i = 1, size(ncolls(:,:),dim=1)
        do j = 1, size(ncolls(:,:),dim=2)
          if(collswitch) final_dtime = min(final_dtime, c1*size(bin(i,j)%p) * old_dt / real(max(1,ncolls(i,j)))) !we don't want more collisions than ~ nr of particles in a zone per timestep
