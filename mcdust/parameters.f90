...
module parameters
    ...
    public :: read_parameters, Ntot, ncell, nr, nz, fout, dtime, tend, smallr, restart, restime, minrad0, maxrad0, &
              ...
              datadir, path, sigmag0, temperature, alphaMRI, erosion_mass_ratio, nbins, mhdwinds, collswitch, &
              dumpfile
    ...
    real                                :: erosion_mass_ratio ! mass ratio between bodies to trigger erosion
    logical                             :: erosion_switch  ! switch on erosion?
    logical                             :: db_data  ! write out files for sanity checks?
    logical                             :: mhdwinds ! switch to use mhd winds gas velocities
    character(len=100)                  :: datadir  ! data directory to write the data into.
    character(len=100)                  :: path     ! absolute path of data directory
    logical                             :: collswitch !switch to turn collisions off
    logical                             :: dumpfile !to write out a data file every timestep (that is overwritten with the newer one)
      ...
      do while (ios == 0)
            ...
            if (ios == 0) then
                ...
                case('erosion_mass_ratio')
                    ...
                    print *, 'erosion mass ratio', erosion_mass_ratio
                ...
                case('mhdwinds')
                    read(buffer, *,iostat=ios) mhdwinds
                    print *, 'mhd winds?', mhdwinds
                case('collisions_on?')
                    read(buffer,*,iostat=ios) collswitch
                    print *, 'collisions on?', collswitch 
                case('write_dumpfile')
                    read(buffer,*,iostat=ios) dumpfile
                    print *, 'write dumpfile?' , dumpfile
