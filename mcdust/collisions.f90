...
module collisions
   ...
   use types,      only: swarm, list_of_swarms, grid_container
   ...
   subroutine mc_collisions(nr, ni, bin, swrm, dtime, realtime, kk, g)
      implicit none
      ...
      do i = 1, nsws
         ...
         call vel_vs_centr(nr, ni, i, stokesnr, Kepler_freq, vs, g)
         if (rdrift) then
            call vel_rd_centr(nr, ni, i, stokesnr, vr, vn, realtime, g)
         endif
      enddo
      ...
      do while (local_time < dtime)
         ...
         call vel_vs_centr(nr, ni, nri, stokesnr, Kepler_freq, vs, g)
         if (rdrift) then
            call vel_rd_centr(nr, ni, nri, stokesnr, vr, vn, realtime, g)
         endif
...
!      !write(*,*) '       swrm updated!'
   ...
   subroutine collision(nri,nrk,swarms,relvels, accelncol)
      ...
      if (rvel < vfrag) then
         ...
         swarms(nri)%stick = swarms(nri)%stick + 1 
      !else
      ...
      !endif
         ...
         swarms(nri)%frag = swarms(nri)%frag + 1
      else
         call fragmentation(nri, swarms)
         swarms(nri)%frag = swarms(nri)%frag + 1
      endif
   ...
   subroutine vel_vs_centr(nr, ni, i, stokesnr, Kepler_freq, vs, g)
      implicit none
   ...
   subroutine vel_rd_centr(nr, ni, i, stokesnr, vr, vn, realtime, g)
      implicit none
      integer, intent(in)                             :: nr, ni
      real, dimension(:), allocatable                 :: stokesnr
      ...
      vr(i) = 2. * vn * stokesnr(i) / (stokesnr(i)**2. + 1.) + &
              vgas(g%rce(nr), g%zce(nr,ni), realtime, .true.) / (1. + stokesnr(i) * stokesnr(i))
